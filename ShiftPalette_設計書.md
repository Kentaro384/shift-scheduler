...
| 今月の準備パネル | 月初作業の進行状況、次に実行すべき操作、未割当の時間指定件数を表示 |
| 月次表 | 職員×日付のシフト表。セルクリックで編集 |
| 出勤人数行 | 日別の出勤人数を表示。不足時は赤表示 |
| 資格者行 | 勤務パターン別の資格者数を表示。不足/過多を色で表示 |
| バランスダッシュボード | 勤務パターン、振休、有給、固定予定の偏り確認 |

今月の準備パネルは、月初作業を `初期化`、`固定勤務`、`固定予定`、`自動生成`、`不足確認`、`Excel` の順で見せる。状態に応じて主ボタンを `固定勤務を反映`、`自動生成する`、`不足を確認`、`Excel出力` に切り替え、利用者が次に何を押せばよいかを画面上で判断できるようにする。未割当の時間指定勤務がある場合は件数を表示し、確認ボタンから最初の対象セルを開けるようにする。
