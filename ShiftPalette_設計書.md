...
| `manualShifts` | 将来拡張または過去設計由来の手動シフト保存領域 |
...
| ID | 意味 | 自動生成時 | Excel |
|---|---|---|---|
| 振 | 振休 | 上書き禁止。土曜出勤時に自動付与されることもある | 固定予定として集計 |
...
| 正規/主任 | `振`, `有`, `半有`, `研`, `出`, `保` だけ保持。それ以外は空欄 |

生成中の `setShift()` は次を守る。

- 固定予定 `振`, `有`, `半有`, `研`, `出`, `保` は上書きしない。
...
| `HourlyStaffChart` | 日別の時間帯別人員ガントチャート |
...
1. `manualShifts` フィールドはFirestore型には存在するが、現行UIの主要フローでは中心的には使われていない。固定予定保護は `schedule` 上の記号で判定している。
...
| 中 | `manualShifts` を実利用し、手動保護の根拠をデータとして分離 | 自動生成結果と手入力の境界をより明確にするため |
