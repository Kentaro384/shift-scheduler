...
| 出 | 出勤(会議出席など)。土曜のみ出勤人数に集計 | 上書き禁止 | 固定予定として集計 |
| 保 | 保育(勤務パターンによらない保育出勤)。出勤人数に集計 | 上書き禁止 | 固定予定として集計 |
...
シフト編集モーダルには「削除」操作がある(2026-07-19追加)。対象セルの `schedule` と `manualShifts` をセル単位の `deleteField()` で削除し、手動入力の保護をそのセルだけ解除して次回自動生成の対象へ戻す。`休` の手動入力(保護される非勤務)とは異なり、削除後のセルは当月白紙化直後と同じ未入力状態になる。監査ログは `manual_shift_delete` で、undoPatch により「戻す」で復元できる。
...
現在表示中の年月、対象月に在籍している職員、スケジュール、時間指定勤務、パターン、祝日、日別備考をもとに `勤務表_YYYY年M月.xlsx` を生成する。
...
- 勤務パターンと固定予定を色付きで表示
- 土日祝の背景色を反映
- 日別備考を4行目の備考行に表示。複数行入力は改行を保持して出力する
- 備考行は高さ148、フォント9pt、折り返し、縦書きで出力する
...
- `振`, `有`, `半有`, `研`, `休`, 空欄は勤務扱いしない
- `保` は終日出勤扱い、`出` は土曜のみ出勤扱い(`countsAsFullDayStaffingShift()`)。土曜会議の日に「保育に入る人(`保`)/会議に出る人(`出`)」を書き分ける実運用に対応する
...
| 備考行 | 日別の行事・メモを表示。セルクリックで複数行入力モーダルを開く |
...
備考行は `notes.YYYY-MM-DD` を表示する。編集は日付セルから開くモーダル内の `textarea` で行い、1日に複数イベントがある場合は改行で区切って保存する。保存は `saveNoteDate()` で対象日だけを更新し、保存失敗時は変更前の備考へロールバックする。
...
| `HelpPage` | ITリテラシーが高くない利用者向けの使い方ガイド。ヘッダー「使い方」ボタンとログイン画面から開ける全画面オーバーレイ。実UIと同じスタイルヘルパーで描画した「画面の再現イメージ」を使うため、静的スクショと違い勤務パターン変更にも追随する |
...
| タイトル | 年・月・勤務表 |
| 日付/曜日 | 土曜は青系、日祝は赤系 |
| 備考行 | 日別備考を改行保持で表示。4行目の高さ148、フォント9pt、折り返し、縦書き |
| 職員列 | 職員名と役職を2段表示 |
...
6. 固定予定 `研` は非勤務扱いである。`保` は出勤扱い、`出` は土曜のみ出勤扱いで、土曜会議の「保育に入る人/会議に出る人」の書き分けに使われている(2026-07-19 利用者確認)。
...
| `src/components/HelpPage.tsx` | 使い方ガイド(全画面オーバーレイ) |
